                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <div class="flex flex-col justify-between flex-1 min-w-[300px]">
                        <div>
                            ...
                            <a href="<?php echo e(route('profile.show', $recipe->user->id ?? '')); ?>" class="inline-flex items-center gap-2 mb-2 px-2 py-1 rounded hover:bg-gray-100">
                                
                                <img 
                                src="<?php echo e($recipe->user->profile_picture ? asset('storage/' . $recipe->user->profile_picture) : asset('gambar/profile.png')); ?>" 
                                alt="profil" 
                        ...
                        <div class="mt-4">
                            <?php if(Auth::check()): ?>
                                <form action="<?php echo e(route('favorites.toggle', $recipe->id)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                                    <button type="submit" class="text-white font-semibold px-4 py-2 rounded-lg flex items-center gap-2 bg-[#F58E4A] hover:bg-[#f56c4a] cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="<?php echo e(Auth::check() && $recipe->favorites()->where('user_id', Auth::id())->exists() ? '#F58E4A' : 'white'); ?>" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 cursor-pointer">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
                                        </svg>
                                        <?php echo e(Auth::check() && $recipe->favorites()->where('user_id', Auth::id())->exists() ? 'Hapus dari Koleksi' : 'Tambahkan ke Koleksi'); ?>

                                    </button>
                                </form>
                            <?php else: ?>
                                <a href="<?php echo e(route('login')); ?>" class="text-white font-semibold px-4 py-2 rounded-lg flex items-center gap-2 bg-[#F58E4A] hover:bg-[#f56c4a] cursor-pointer w-fit">
