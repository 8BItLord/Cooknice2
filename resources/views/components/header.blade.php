<header class="flex space-x-4 my-2 justify-end sticky top-0 bg-white py-2 z-10">
    @auth
        <button popovertarget="clickedprofile" class="rounded-full">
            <img src="{{ Auth::user()->profile_picture ? asset('storage/' . Auth::user()->profile_picture) : asset('gambar/profile.png') }}" alt="profile" class="w-[50px] h-[50px] cursor-pointer border-2 rounded-full border-[#F58E4A]">
        </button>
